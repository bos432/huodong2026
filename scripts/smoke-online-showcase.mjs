...
async function mallFlow(financeToken, opsToken) {
  ...
  const filterStartDate = new Date(today.getTime() - 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
  const filterEndDate = new Date(today.getTime() + 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
  const fulfillmentPaymentMethod = fulfillmentOrder.paymentMethod || "offline";
  const filteredOrderResult = await api(`/admin/mall/orders?paymentMethod=${fulfillmentPaymentMethod}&refundStatus=approved&startDate=${filterStartDate}&endDate=${filterEndDate}&keyword=${encodeURIComponent(fulfillmentOrder.orderNo)}${merchantQuery}`, { headers: auth(financeToken) });
  assert(pickList(filteredOrderResult).some((item) => item.orderNo === fulfillmentOrder.orderNo), "商城订单筛选应支持支付方式、售后状态和日期");
  const filteredSummary = await api(`/admin/mall/orders/summary?paymentMethod=${fulfillmentPaymentMethod}&refundStatus=approved&startDate=${filterStartDate}&endDate=${filterEndDate}&keyword=${encodeURIComponent(fulfillmentOrder.orderNo)}${merchantQuery}`, { headers: auth(financeToken) });
  ...
  const filteredExportRes = await fetch(`${API_BASE}/admin/mall/orders/export?paymentMethod=${fulfillmentPaymentMethod}&refundStatus=approved&startDate=${filterStartDate}&endDate=${filterEndDate}&keyword=${encodeURIComponent(fulfillmentOrder.orderNo)}${merchantQuery}`, { headers: auth(financeToken) });
