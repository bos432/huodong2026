...
线上演示商家账号由 `scripts/seed-online-showcase.mjs` 幂等创建/重置，密码通过部署环境变量 `SHOWCASE_PASSWORD` 注入，不写入公开文档：

| 账号 | 角色定位 | 验收重点 |
| --- | --- | --- |
| `showcase_admin` | 商家管理员 | 商家核心配置、活动、报名、会员、商城、装修营销 |
| `showcase_ops` | 运营人员 | 活动、报名、会员、装修、弹窗、广告 |
| `showcase_finance` | 财务人员 | 订单收款、退款、财务对账、商城财务、代理结算 |
| `showcase_checkin` | 签到人员 | 报名查询、现场签到核销 |
| `showcase_store_owner` | 店铺负责人 | 可管理授权店铺、商品、商城订单，不处理平台账号授权 |
| `showcase_store_finance` | 店铺财务 | 可查看/处理店铺订单、结算、收款状态，不可编辑商品 |
| `showcase_agent_owner` | 代理负责人 | 可查看相关订单和代理结算，不可编辑商城商品 |

部署后可执行 `node scripts/verify-online-showcase-roles.mjs` 验证店铺/代理演示账号登录和关键权限边界。
...
- 抽查运营、财务、签到账号权限。
- 抽查店铺负责人、店铺财务、代理负责人权限，确认店铺财务不能编辑商品、代理负责人不能进入商品编辑。
- 抽查操作日志和后台登录失败记录。
