...
- 已发布运行版本：`2057a5e2e5f97aaf5f8f6dfca815fac0e1867c87`。
- 数据库迁移：本次无新增迁移，不执行迁移命令。
- 变更：平台级订单在机构会员视图可见、订单页状态稳定、线下收款转化统计补齐、会员中心订单状态改为待处理/待参与/待学习/已完成。
...
本次导入后确认 `version.json` 为 `2057a5e2`，真机验证首页底部导航、活动详情、我的订单、待参与订单、订单详情和跨租户平台订单。
