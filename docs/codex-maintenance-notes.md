...
- 小程序按“线下活动/预约报名/会议展览”类目提交时，前台不要出现“课程、题库、答案、培训、教育、售价、报名费、付费课程”等容易触发教育类目或安全风控的表达。
- 历史课程功能的接口路径可以暂时保留，但小程序前台展示统一改为“专题内容、活动资料、参与权益、观看进度”等中性文案。
- 首页装修、默认落地页和演示 seed 都要一起改；只改页面硬编码不够，线上装修数据也可能把旧词渲染出来。
- 如线上数据库里已有活动标题/装修文案包含敏感词，优先在后台手工改；小程序端已有展示层兜底，但正式交付前仍建议清理数据源。
