...
const uploadingField = ref<"iconUrl" | "coverUrl" | "">("");
const errorMessage = ref("");
const editingId = ref<number | null>(null);
const form = reactive({ name: "", iconUrl: "", coverUrl: "", publicVisible: true, scene: "activity", sortOrder: 0, enabled: true });
const canManage = computed(() => hasPermission("category.manage"));

const roleHint = computed(() => !canManage.value
  ? "当前账号仅可查看活动分类，新增、编辑、启停和图片上传均不可用。"
  : isPlatformAdmin()
    ? "平台账号维护平台自营/全局分类，默认会展示在平台 H5 分类区。商家自己的分类由商家账号在商家端维护。"
...
function openCreate() {
  if (!canManage.value) return ElMessage.warning("当前账号只能查看分类");
...
async function submit() {
  if (!canManage.value) return ElMessage.warning("当前账号只能查看分类");
  if (saving.value || actionId.value !== null || uploadingField.value) return;
...
function beforeImageUpload(field: "iconUrl" | "coverUrl") {
  return (file: File) => {
  ...
  <div class="page">
    <div class="toolbar">
      ...
      <div class="toolbar-actions">
        ...
        <el-button v-if="canManage" type="primary" :icon="Plus" :disabled="saving || actionId !== null" @click="openCreate">新增分类</el-button>
      </div>
    </div>
    ...
    <el-alert v-if="errorMessage" class="load-error" type="error" :closable="false" show-icon :title="errorMessage">
      <template #default><el-button size="small" :loading="loading" @click="load">重新加载分类</el-button></template>
    </el-alert>
    <div class="table-card">
      <el-table v-loading="loading" :data="rows" stripe empty-text="暂无分类">
        ...
        <el-table-column v-if="canManage" label="操作" width="180" fixed="right">
          <template #default="{ row }">
            <el-button size="small" :icon="Edit" :disabled="actionId !== null || saving" @click="openEdit(row)">编辑</el-button>
            <el-button size="small" :type="row.enabled ? 'warning' : 'success'" :icon="Switch" :loading="actionId === row.id" :disabled="actionId !== null || saving" @click="toggle(row)">{{ row.enabled ? "停用" : "启用" }}</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    <el-dialog v-model="dialog" width="620px" :title="editingId ? '编辑分类' : '新增分类'" destroy-on-close>
      <el-form label-position="top">
        ...
        <el-form-item label="图标">
          <div class="image-field">
            ...
            <el-upload action="/api/admin/uploads/images" name="file" :headers="uploadHeaders()" :show-file-list="false" :disabled="Boolean(uploadingField)" :before-upload="beforeImageUpload('iconUrl')" :on-success="handleUploadSuccess('iconUrl')" :on-error="handleUploadError">
              <el-button :icon="UploadFilled" :loading="uploadingField === 'iconUrl'" :disabled="Boolean(uploadingField)">上传图标</el-button>
            </el-upload>
        ...
        <el-form-item label="封面">
          <div class="image-field">
            ...
            <el-upload action="/api/admin/uploads/images" name="file" :headers="uploadHeaders()" :show-file-list="false" :disabled="Boolean(uploadingField)" :before-upload="beforeImageUpload('coverUrl')" :on-success="handleUploadSuccess('coverUrl')" :on-error="handleUploadError">
              <el-button :icon="UploadFilled" :loading="uploadingField === 'coverUrl'" :disabled="Boolean(uploadingField)">上传封面</el-button>
            </el-upload>
      ...
      <template #footer>
        <el-button :disabled="saving || Boolean(uploadingField)" @click="dialog = false">取消</el-button>
        <el-button type="primary" :loading="saving" :disabled="Boolean(uploadingField)" @click="submit">保存</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
@media (max-width: 640px) {
  ...
  .image-field { grid-template-columns: minmax(0, 1fr); }
  .preview-cover { width: 100%; max-width: 220px; }
}
</style>
