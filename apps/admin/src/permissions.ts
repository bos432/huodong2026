...
export const adminPermissionGroups = [
  ...
  { group: "系统安全", items: [{ key: "logs.view", label: "操作/登录/验证码日志" }, { key: "system.manage", label: "系统设置/上线体检" }] },
  ...
  { group: "书院运营", items: [{ key: "course.manage", label: "课程管理", platformOnly: true }, { key: "community.manage", label: "共修管理", platformOnly: true }] }
] as const;
...
export function defaultPermissionsForRole(role?: string | null, tenantScoped = false) {
  ...
  if (normalized === AdminRole.Operator) return [
    ...
    "homepage.manage", "announcement.manage", "operation_settings.manage", "tenant_profile.manage", "charity.view", "charity.manage", "upload.image"
  ];
  if (normalized === AdminRole.Finance) return [
    ...
    "agent_settlement.manage", "agent_settlement.pay", "agent_settlement.transfer", "agent_settlement.export", "charity.view", "charity.finance", "upload.settlement_proof"
  ];
