...
function render() {
  ...
  if (state.step === 3) el("content").innerHTML = '<div class="grid">' + field("admin","username","超级管理员账号") + field("admin","password","超级管理员密码","password") + '</div><div class="notice">密码至少 10 位。安装完成后请妥善保存。</div>';
  if (state.step === 4) el("content").innerHTML = '<div class="muted">短信和微信支付可以跳过，安装后在后台或 env 中继续配置。螺丝帽短信填写 API Key 和签名即可；真实微信支付上线前仍需专项验收。</div><div class="grid">' + field("optional","smsAccessKeyId","短信 Key（螺丝帽可留空）") + field("optional","smsAccessKeySecret","短信 Secret/API Key","password") + field("optional","smsSignName","短信签名") + field("optional","smsTemplateId","短信模板 ID（腾讯云）") + field("optional","wechatAppId","微信 AppId") + field("optional","wechatAppSecret","微信 AppSecret","password") + field("optional","wechatPayMchId","微信商户号") + field("optional","wechatPayApiV3Key","微信 API v3 Key","password") + '</div>';
