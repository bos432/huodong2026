...
export class MemberPointLedgerGovernance1783880000000 implements MigrationInterface {
  ...
  async up(queryRunner: QueryRunner) {
    ...
    await queryRunner.query(`UPDATE member_point_logs p JOIN refunds r ON r.refundNo = p.sourceId JOIN orders o ON o.id = r.orderId SET p.tenantId = o.tenantId, p.tenantScopeKey = IF(o.tenantId IS NULL, 'platform', CONCAT('tenant:', o.tenantId)) WHERE p.sourceType = 'order_refund'`);
    ...
    await queryRunner.query(`UPDATE member_point_logs p JOIN mall_orders o ON o.id = CAST(p.sourceId AS UNSIGNED) AND o.userId = p.userId SET p.tenantId = o.tenantId, p.tenantScopeKey = IF(o.tenantId IS NULL, 'platform', CONCAT('tenant:', o.tenantId)) WHERE p.sourceType = 'mall_order_refund' AND o.createdAt <= p.createdAt AND NOT EXISTS (SELECT 1 FROM mall_refunds candidate WHERE (candidate.id = CAST(p.sourceId AS UNSIGNED) OR candidate.refundNo = p.sourceId) AND candidate.userId = p.userId AND candidate.createdAt <= p.createdAt)`);
    await queryRunner.query(`UPDATE member_point_logs p JOIN mall_refunds r ON (r.id = CAST(p.sourceId AS UNSIGNED) OR r.refundNo = p.sourceId) AND r.userId = p.userId JOIN mall_orders o ON o.id = r.orderId SET p.tenantId = o.tenantId, p.tenantScopeKey = IF(o.tenantId IS NULL, 'platform', CONCAT('tenant:', o.tenantId)) WHERE p.sourceType = 'mall_order_refund'`);
    ...
    await queryRunner.query(`UPDATE member_point_logs claw JOIN refunds refund ON refund.refundNo = claw.sourceId JOIN member_point_logs earned ON earned.sourceType = 'order_paid' AND earned.sourceId = CAST(refund.orderId AS CHAR) AND earned.userId = claw.userId AND earned.tenantScopeKey = claw.tenantScopeKey SET claw.relatedLogId = earned.id WHERE claw.sourceType = 'order_refund'`);
    ...
    await queryRunner.query(`UPDATE member_point_logs claw JOIN mall_orders businessOrder ON businessOrder.id = CAST(claw.sourceId AS UNSIGNED) AND businessOrder.userId = claw.userId JOIN member_point_logs earned ON earned.sourceType = 'mall_order_paid' AND earned.sourceId = CAST(businessOrder.id AS CHAR) AND earned.userId = claw.userId AND earned.tenantScopeKey = claw.tenantScopeKey SET claw.relatedLogId = earned.id WHERE claw.sourceType = 'mall_order_refund' AND claw.relatedLogId IS NULL AND businessOrder.createdAt <= claw.createdAt AND NOT EXISTS (SELECT 1 FROM mall_refunds candidate WHERE (candidate.id = CAST(claw.sourceId AS UNSIGNED) OR candidate.refundNo = claw.sourceId) AND candidate.userId = claw.userId AND candidate.createdAt <= claw.createdAt)`);
    await queryRunner.query(`UPDATE member_point_logs claw JOIN mall_refunds refund ON (refund.id = CAST(claw.sourceId AS UNSIGNED) OR refund.refundNo = claw.sourceId) AND refund.userId = claw.userId JOIN member_point_logs earned ON earned.sourceType = 'mall_order_paid' AND earned.sourceId = CAST(refund.orderId AS CHAR) AND earned.userId = claw.userId AND earned.tenantScopeKey = claw.tenantScopeKey SET claw.relatedLogId = earned.id WHERE claw.sourceType = 'mall_order_refund' AND claw.relatedLogId IS NULL`);
