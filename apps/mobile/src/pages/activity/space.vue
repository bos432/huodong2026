<template>
  <view class="space-page has-custom-nav">
    <view v-if="loading" class="space-state" role="status" aria-live="polite">活动空间加载中…</view>
    <view v-else-if="error" class="space-state error" role="alert" aria-live="assertive"><text>{{ error }}</text><text class="retry" role="button" tabindex="0" @click="load" @keyup.enter="load" @keyup.space.prevent="load">重新加载</text></view>
    <template v-else-if="space">
      ...
      <view class="member-strip app-enter" :style="motionStyle(82)">
      ...
      <view class="space-section app-enter" :style="motionStyle(118)">
      ...
      <view class="space-section app-enter" :style="motionStyle(150)">
      ...
      <view class="space-section app-enter" :style="motionStyle(184)">
      ...
      <view class="space-section action-section app-enter" :style="motionStyle(218)">
    ...
    <view v-if="composerVisible" class="composer-mask" @click.self="composerVisible = false"><view class="composer app-sheet-enter" @click.stop><text class="composer-title">提出问题</text><textarea v-model="draft" maxlength="1000" placeholder="活动安排、现场准备等问题都可以写在这里" auto-height /><view class="composer-actions"><text class="app-press" @click.stop="composerVisible = false">取消</text><text class="submit app-press" :class="{ disabled: posting }" @click.stop="submitPost">{{ posting ? "发布中..." : "发布" }}</text></view></view></view>
  </view>
</template>
...
const id = ref(0); const space = ref<any>(null); const shareInvite = ref<any>(null); const loading = ref(true); const error = ref(""); const composerVisible = ref(false); const posting = ref(false); const draft = ref(""); const groupQrImageError = ref(false); const stageNow = ref(Date.now());
...
function openComposer() { if (!posting.value) composerVisible.value = true; }
async function submitPost() {
  const content = draft.value.trim();
  if (posting.value) return;
  if (!content || !id.value) return uni.showToast({ title: "请填写问题", icon: "none" });
  posting.value = true;
  try {
    const post = await request(`/public/activities/${id.value}/space/posts`, { method: "POST", data: { content } });
    if (space.value) {
      if (!Array.isArray(space.value.posts)) space.value.posts = [];
      space.value.posts.unshift(post);
    }
    draft.value = "";
...
.space-page{background:var(--app-page-bg)}
.space-hero{height:380rpx;background:#16252d}
.member-strip,.space-section{border-color:var(--app-border);border-radius:16rpx;box-shadow:none}
.post,.empty{border-radius:12rpx;background:#f5f7f8}
.reply{border-radius:10rpx;background:#eaf7f1}
.action-cell{border:1rpx solid var(--app-border);border-radius:14rpx;background:#fff;color:var(--app-text)}
.composer{border-radius:20rpx 20rpx 0 0}
.composer textarea{border:1rpx solid var(--app-border);border-radius:14rpx;background:#fff}
</style>
