<template>
  <view class="profile-page has-custom-nav">
    ...
    <view class="card charity-card" @click="goCharity">
      <view class="row">
        <view>
          ...
          <text style="font-size:24rpx; color:#999; display:block; margin-top:4rpx;">累计贡献 0 元</text>
        </view>
    ...
    <view class="card" style="margin-bottom:16rpx;" @click="goWallet">
      ...
      <text style="font-size:40rpx; color:#C43D3D; font-weight:700; margin-top:8rpx; display:block;">¥0.00</text>
    </view>

    <!-- 手机管理入口（有权限时） -->
    <view class="card" style="margin-bottom:16rpx;" @click="goAdmin">
      <view class="row">
        <text style="font-size:30rpx; font-weight:600; color:#333;">📱 手机管理</text>
        <text class="subtle">活动管理 · 报名审核 · 签到核销</text>
      </view>
    </view>
...
const orderTabs = [
  { icon:"💳", label:"待付款", count:0, status:"pending" },
  { icon:"📚", label:"待学习", count:0, status:"learning" },
  { icon:"✅", label:"已完成", count:0, status:"completed" },
  { icon:"📋", label:"全部", count:0, status:"all" }
];
...
.profile-header {
  background: linear-gradient(180deg, #F5F0E8, #E8E0D8);
  display: flex;
