...
const props = defineProps<{ current: "home" | "activities" | "registrations" | "orders" | "refunds" | "analytics" | "checkin"; permissions?: Record<string, any> | null }>();
...
const items = [
  ...
  { key: "refunds", label: "退款", icon: "退", url: "/pages/admin/refunds", permission: "canViewRefunds" },
  { key: "analytics", label: "统计", icon: "统", url: "/pages/admin/analytics", permission: "canViewAnalytics" },
...
const visibleItems = computed(() => items.filter(visible));
...
<template>
  <view class="admin-nav-wrap">
    <view class="admin-bottom-nav" :style="{ gridTemplateColumns: `repeat(${Math.max(visibleItems.length, 1)}, minmax(0, 1fr))` }">
      <view v-for="item in visibleItems" :key="item.key" class="nav-item" :class="{ active: item.key === current }" @click="go(item)">
...
.admin-bottom-nav { display: grid; gap: 6rpx; padding: 12rpx; border: 1rpx solid rgba(196, 61, 61, 0.12); border-radius: 34rpx; background: rgba(255, 252, 247, 0.96); box-shadow: 0 -12rpx 34rpx rgba(91, 47, 36, 0.14); backdrop-filter: blur(16px); pointer-events: auto; }
...
.nav-label { white-space: nowrap; font-size: 20rpx; }
